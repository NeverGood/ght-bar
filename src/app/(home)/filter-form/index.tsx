...
export default function Form({
    ...
    const {
        ...
        defaultValues: { collection, type, strength, name, countryOrigin, user },
        resolver: zodResolver(FilterSchema),
    });

    const onSubmit = useSubmit(setQuery);
    const hasActiveFilters = Boolean(
        collection || type || strength || name || countryOrigin || user
    );

    return (
        <form
            className="grid grid-cols-6 gap-3 xl:grid-cols-3 lg:grid-cols-2 md:grid-cols-1"
        ...
        >
            <Select
                ...
                id="collection"
                label="Коллекция"
                options={filterOptions.collections.map((value) => ({
                    label: value,
                    value,
                }))}
                placeholder="Любая коллекция"
            />
            <Select
                {...register("type")}
